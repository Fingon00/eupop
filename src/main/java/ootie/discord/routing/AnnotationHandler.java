...
import ootie.discord.listeners.context.ModalContext;
import ootie.discord.listeners.context.SelectionMenuContext;
    ...
    static List<Class<?>> getAllClasses() {
        if (classes.isEmpty()) {
            ...
            reflections.get(SubTypes.of(Object.class).asClass()).stream()
                    .filter(c -> c.getPackageName().startsWith("ootie"))
                    .forEach(classes::add);
        }
        return classes;
    }
