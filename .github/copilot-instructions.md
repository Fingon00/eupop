...
src/
  main/
    java/ootie/
      OotieBot.java     # Spring Boot entry point
...
- Handlers are discovered via classpath scanning (Reflections library) of the `ootie` package.
...
- **`Mapper`** (in `ootie.image`): static utility class; loaded at startup via `Mapper.init()`. Provides access to all game content: factions, tiles, planets, technologies, colors, leaders, etc.
...
- All `Model` classes (in `ootie.model`) are immutable data containers loaded from JSON.
...
- `BotLogger` (in `ootie.message.logging`) is the primary logger — use it instead of raw `System.out` or SLF4J.
...
Scheduled tasks live in `ootie.cron.*`. They are started by `JdaService` after bot initialization.
...
| New button handler | Annotate method with `@ButtonHandler("prefix")` anywhere in `ootie.*` |
